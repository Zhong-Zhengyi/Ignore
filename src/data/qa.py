...
class QAwithIdkDataset(QADataset):
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(item, dict):
            ...
            return_item = {"original": item[0]}  # 只取第一个答案
            idk_item = self.item_with_idk(question)
            return_item["alternate"] = idk_item
        else:
            raise NotImplementedError(f"Unexpected item type: {type(item)}")
        return return_item


class QAwithGenerate(QADataset):
    def __init__(self, model, return_original=True, *args, **kwargs):
        self.device = "cuda" if torch.cuda.is_available() else "cpu"  # 自动使用GPU
        self.model = model.to(self.device)
        self.tokenizer = kwargs.get("tokenizer", None)
        self.return_original = return_original
        super().__init__(*args, **kwargs)

    def item_with_generate(self, question):
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(item, dict):
            return_item = {"original": item}
            idk_item = self.item_with_generate(question)
            return_item["alternate"] = idk_item
        elif isinstance(item, list) or isinstance(item, tuple):
            return_item = {"original": item[0]}  # 只取第一个答案
            idk_item = self.item_with_generate(question)
    ...
    def generate_answer(self, model, tokenizer, question, max_length=200):
