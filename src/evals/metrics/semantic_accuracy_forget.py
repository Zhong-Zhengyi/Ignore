...
class QwenAPIEvaluator:
    """使用qwen-max模型API进行语义评估"""
    def __init__(self, api_keys: list, base_url: str = "https://dashscope.aliyuncs.com/api/v1"):
        self.api_keys = api_keys
        self.base_url = base_url
        self.model_name = "qwen-max-2025-01-25"
        self.key_index = 0  # 当前使用的key索引
    ...
    def call_api(self, prompt: str, max_retries: int = 3) -> str:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                # 在成功调用后短暂sleep，降低请求频率
                time.sleep(0.5)
                return result["output"]["text"].strip()
            except Exception as e:
                logger.warning(f"API调用失败 (key: {api_key}, 尝试 {attempt + 1}/{max_retries}): {e}")
...
def semantic_accuracy_forget(model, **kwargs):
    ...
    if isinstance(api_key, str):
        ...
        api_keys = api_key  # 已经是列表
    api_evaluator = QwenAPIEvaluator(api_keys=api_keys)
